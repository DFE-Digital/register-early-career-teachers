...
module RSpecPlaywright
  ...
  def self.start_browser
    ...
    session = execution.playwright
    browser = session.chromium.launch(headless: true)
    browser.new_page(baseURL: RAILS_SERVER_URL)
  end
  ...
  def self.stop_rails_server(pid: RSpec.configuration.rails_server_pid)
