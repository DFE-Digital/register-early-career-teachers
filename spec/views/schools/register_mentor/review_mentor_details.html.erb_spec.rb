...
RSpec.describe "schools/register_mentor/review_mentor_details.html.erb" do
  let(:back_path) { schools_register_mentor_find_mentor_path }
  let(:date_of_birth) { "1950/1/1" }
  let(:confirm_and_continue_path) { schools_register_mentor_review_mentor_details_path }
  ...
  let(:wizard) { Schools::RegisterMentor::Wizard.new(current_step: :review_mentor_details, store:) }
