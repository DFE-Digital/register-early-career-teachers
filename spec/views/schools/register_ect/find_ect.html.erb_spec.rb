RSpec.describe "schools/register_ect/find_ect.html.erb" do
  let(:back_path) { schools_register_ect_start_path }
  let(:continue_path) { schools_register_ect_find_ect_path }
  let(:step) { Schools::RegisterECT::FindECTStep.new }
  let(:title) { "Find an ECT" }
  let(:wizard) { Schools::RegisterECT::Wizard.new(current_step: :find_ect, store: {}) }
